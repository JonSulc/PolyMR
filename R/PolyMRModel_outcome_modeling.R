...
max(exposure_powers) = %i, max_exposure_power = %i",
  ...
  update_control_function_powers(polymr_model,
...
update_control_function_powers <- function(polymr_model,
  ...
  if (!is.null(control_function_powers))
    ...
    polymr_model$control_function_powers <-
      1:min(max(polymr_model$exposure_powers, 1), polymr_model$max_control_function_power)

  if (update_outcome_model &&
      !setequal(old_control_function_powers, polymr_model$control_function_powers))
